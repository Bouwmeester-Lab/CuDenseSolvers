...
TEST(SolverTests, TestBiCGStabWorkspaceEdgeCases) {
	// Test invalid size
	auto [rawMemory1, workspace1] = allocateBiCGStabWorkspace(0);
	EXPECT_EQ(rawMemory1, nullptr);
	EXPECT_FALSE(workspace1.isValid());
	
	auto [rawMemory2, workspace2] = allocateBiCGStabWorkspace(-1);
	EXPECT_EQ(rawMemory2, nullptr);
	EXPECT_FALSE(workspace2.isValid());
	
	// Test constructor with null pointer
	BiCGStabWorkspace workspace3(nullptr, 10);
	EXPECT_FALSE(workspace3.isValid());
	EXPECT_EQ(workspace3.n, 0);
}